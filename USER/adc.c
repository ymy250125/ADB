 ...
 void ADC1_Mode_Config(void)
{
  ...
  DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)ADCConvertedValue;	//设置DMA内存地址，ADC转换结果直接放入该地址
  DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC; //外设为设置为数据传输的来源
  DMA_InitStructure.DMA_BufferSize = 5;	//DMA缓冲区设置为1；
  ...
  ADC_InitStructure.ADC_NbrOfChannel = 5;  // 只有1个转换通道
  ADC_Init(ADC1, &ADC_InitStructure);

  /* ADC1 regular channel11 configuration */ 
  //ADC_RegularChannelConfig(ADC1, ADC_Channel_0, 1, ADC_SampleTime_55Cycles5); //通道1采样周期55.5个时钟周期
  ADC_RegularChannelConfig(ADC1, ADC_Channel_1, 1, ADC_SampleTime_1Cycles5); //通道1采样周期1.5个时钟周期
  ADC_RegularChannelConfig(ADC1, ADC_Channel_2, 2, ADC_SampleTime_1Cycles5); //通道1采样周期1.5个时钟周期
	ADC_RegularChannelConfig(ADC1, ADC_Channel_3, 3, ADC_SampleTime_1Cycles5); //通道1采样周期1.5个时钟周期
	ADC_RegularChannelConfig(ADC1, ADC_Channel_6, 4, ADC_SampleTime_1Cycles5); //通道1采样周期1.5个时钟周期
	ADC_RegularChannelConfig(ADC1, ADC_Channel_7, 5, ADC_SampleTime_1Cycles5); //通道1采样周期1.5个时钟周期
